...
public class CuratorClusterBaseOperations extends CuratorClusterBase {
    ...
    public void testCluster() throws Exception {
        ...
        log.info("###################bytes111 {}", new String(bytes));
        while (true) {

            try {
                byte[] bytes2 = curatorFramework.getData().forPath(pathWithParent);
                System.out.println(new String(bytes2));
                log.info("###################bytes222 {}", new String(bytes2));
    ...
    public static void main(String[] args) throws Exception {
        ...
        String connectStr = "192.168.60.120:2181,192.168.60.120:2182,192.168.60.120:2183,192.168.60.120:2184";
        ...
        String pathWithParent = "/test";
        byte[] bytes = curatorFramework.getData().forPath(pathWithParent);
        System.out.println("bytes111" + new String(bytes));
        log.info("bytes111 {}", new String(bytes));
        while (true) {

            try {
                byte[] bytes2 = curatorFramework.getData().forPath(pathWithParent);
                System.out.println("bytes222" + new String(bytes2));
                log.info("bytes222 {}", new String(bytes2));
                TimeUnit.SECONDS.sleep(5);
            } catch (Exception e) {
                e.printStackTrace();
